...
import { LineChart, Line, XAxis, YAxis,Tooltip, CartesianGrid, Legend, Label, LabelList } from 'recharts';
...
export default class FundChart extends Component {
  ...
  fetchData = (body) => {
    let resource = '/api/statistics/funds/polyline'
    const cb = (json)=>{
      let nextState = {
        loading: false
      }
      if(json.error){
        ...
        let rechargePoints = json.data.rechargePoints, withdrawPoints = json.data.withdrawPoints
        ...
        if(timeUnit===2){
          ...
          rechargePoints&&rechargePoints.map((r,i)=>{
          ...
          withdrawPoints&&withdrawPoints.map((r,i)=>{
            let x = Format.dayFormat(r.x)
            let xInData = data.find((r,i)=>(r.x===x))
            if(xInData){
              xInData.withdraw= r.y//push shower points into data array
            }
          })
        }else{
  ...
  fetchCompareData = () => {
    ...
    const cb = (json)=>{
      ...
      if(json.error){
        ...
        let rechargePoints = json.data.rechargePoints, withdrawPoints = json.data.withdrawPoints, newData = JSON.parse(JSON.stringify(this.state.data))
        if(timeSpan===1){
          newData.map((r,i)=>{
            r.lastReceive = 0
            r.lastWithdraw=0
  ...
  render() {
    const { data,startTime,endTime, selectedSchool, timeUnit,loading,target,timeSpan,compare } = this.state;
        ...
        <div >
          ...
          <div className='query'>
            <div>
              ...
              <a data-value={1} className={target===1?'padLR on':'padLR'} onClick={this.changeTarget}>订单数量</a>
              <a data-value={2} className={target===2?'on':''} onClick={this.changeTarget}>订单金额</a>
            </div> 
            <div>
          ...
          <div className="lineChartWrapper">
            ...
            >
              ...
              <Line name={compare?`${DAY[timeSpan]}充值${amountOrProfit}`:`充值${amountOrProfit}`} dataKey="y" stroke="#4aaaef" dot={false} activeDot={{strokeWidth: 2}} />
              <Line name={compare?`${DAY[timeSpan]}提现${amountOrProfit}`:`提现${amountOrProfit}`} dataKey='withdraw' stroke='#97da7b' dot={false} activeDot={{strokeWidth: 2}} />
              {compare?<Line name={compare?`${LASTDAY[timeSpan]}充值${amountOrProfit}`:`充值${amountOrProfit}`} dataKey='lastReceive' stroke='#ffa312' dot={false} activeDot={{strokeWidth: 2}} />:null}
              {compare?<Line name={compare?`${LASTDAY[timeSpan]}提现${amountOrProfit}`:`提现${amountOrProfit}`} dataKey='lastWithdraw' stroke='#ff5555' dot={false} activeDot={{strokeWidth: 2}} />:null}
            </LineChart>
          </div>
        </div>
      </div>
