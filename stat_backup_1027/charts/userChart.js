...
export default class UserChart extends Component {
  ...
  fetchData = (body) => {
    ...
    const cb = (json)=>{
      ...
      if(json.error){
        ...
        let newlyPoints = json.data.newlyPoints,data
        ...
        if(timeUnit===2){
          ...
          newlyPoints&&newlyPoints.map((r,i)=>{
  ...
  fetchCompareData = () => {
    ...
    const cb = (json)=>{
      ...
      if(json.error){
        ...
        let newlyPoints = json.data.newlyPoints, newData = JSON.parse(JSON.stringify(this.state.data))
        ...
        if(timeSpan===1){
          ...
          newlyPoints&&newlyPoints.map((r,i)=>{
            ...
            let item = newData.find((record,ind)=>(Format.dayFormat(r.x)===record.lastX))
            if(item){
              item.lastUser = r.y
            }
          })
  ...
  changeTimeSpan = (e) => {
    ...
    let {startTime,endTime,timeUnit,selectedSchool,timeSpan,compare} = this.state
    if(v===timeSpan){
      return
    }
    const body = {}
  ...
  render() {
    const { data, selectedSchool, startTime,endTime,timeUnit,loading,timeSpan,compare,totalAmount } = this.state;
    return (
      <div className='chart'>
        <h1>
          用户统计
        ...
        <div >
          ...
          <div className="lineChartWrapper">
            ...
            >
              ...
              <YAxis allowDecimals={false} axisLine={{stroke:'#ddd'}} domain={[0, 'dataMax']} tickLine={false} tick={<CustomizedYAxisTick />} />
              ...
              <Line name={compare?`${DAY[timeSpan]}新增用户`:'新增用户'} dataKey="y" stroke="#4aaaef" dot={false} activeDot={{strokeWidth: 2}} />
              {compare?<Line name={compare?`${LASTDAY[timeSpan]}新增用户`:'新增用户'} dataKey='lastUser' stroke='#ffa312' dot={false} activeDot={{strokeWidth: 2}} />:null}
            </LineChart>
          </div>
        </div>
      </div>
...
const CustomizedTooltip = React.createClass({
  render () {
    ...
    const curItems = !!cur&&cur.map((r,i)=>{
        if(r.payload.x){
          return (
              <li key={i}>
    ...
    const lastItems = !!last.length&&last.map((r,i)=>{
        return (
            <li key={i}>
              <svg key={`svg${i}`} className='lineIcon' >
                <line x1="0" x2="5" y1={5} y2={5} stroke={r.stroke} />
      ...
      return (
        <div className='tooltip'>
          <ul>
            ...
            {!!cur?curItems:null}
            {!!last.length?(<li className='label'>{last[0].payload.lastX}</li>):null}
