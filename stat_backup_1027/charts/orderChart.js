...
export default class Chart extends Component {
  ...
  fetchData = (body) => {
    ...
    const cb = (json)=>{
      ...
      if(json.error){
        ...
        let showerPoints = json.data.showerPoints, waterPoints = json.data.waterPoints
        ...
        if(timeUnit===2){
          ...
          showerPoints&&showerPoints.map((r,i)=>{
          ...
          waterPoints&&waterPoints.map((r,i)=>{
            let x = Format.dayFormat(r.x)
            let xInData = data.find((r,i)=>(r.x===x))
            if(xInData){
  ...
  fetchCompareData = () => {
    ...
    const cb = (json)=>{
      ...
      if(json.error){
        ...
        let showerPoints = json.data.showerPoints, waterPoints = json.data.waterPoints, newData = JSON.parse(JSON.stringify(this.state.data)),timeSpan=this.state.timeSpan
        ...
        if(timeSpan===1){
          ...
          showerPoints&&waterPoints.map((r,i)=>{
  ...
  componentWillReceiveProps (nextProps) {
    let {startTime, endTime, selectedSchool, timeUnit, compare} = this.props, body = {}, nextState = {}
    if (nextProps.selectedSchool !== selectedSchool) {
      ...
      this.fetchData(body)
      this.setState(nextState)
    } else if (nextProps.startTime !== startTime || nextProps.endTime !== endTime) {
        ...
        <div >
          ...
          <div className='query'>
            <div>
              ...
              <a data-value={2} className={target===2?'on':''} onClick={this.changeTarget}>订单收益</a>
            </div> 
          </div>
          ...
          <div className="lineChartWrapper">
            ...
            >
              ...
              <Line name={compare?`${DAY[timeSpan]}热水器${amountOrProfit}`:`热水器${amountOrProfit}`} dataKey="y" label='饮水机'  stroke="#4aaaef" dot={false} activeDot={{strokeWidth: 2}} />
              <Line name={compare?`${DAY[timeSpan]}饮水机${amountOrProfit}`:`饮水机${amountOrProfit}`} dataKey='water' stroke='#97da7b' dot={false} activeDot={{strokeWidth: 2}} />
              {compare?<Line name={compare?`${LASTDAY[timeSpan]}热水器${amountOrProfit}`:`热水器${amountOrProfit}`} dataKey='lastShower' stroke='#ffa312' dot={false} activeDot={{strokeWidth: 2}} />:null}
              {compare?<Line name={compare?`${LASTDAY[timeSpan]}饮水机${amountOrProfit}`:`饮水机${amountOrProfit}`} dataKey='lastWater' stroke='#ff5555' dot={false} activeDot={{strokeWidth: 2}} />:null}
            </LineChart>
          </div>
        </div>
      </div>
