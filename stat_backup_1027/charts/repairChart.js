...
export default class RepairChart extends Component {
    ...
    const cb = (json)=>{
      ...
      if(json.error){
        ...
        if(timeUnit===2){
          ...
          showerPoints&&showerPoints.forEach((r,i)=>{
          ...
          waterPoints&&waterPoints.forEach((r,i)=>{
            let x = Format.dayFormat(r.x)
            let xInData = data.find((r,i)=>(r.x===x))
            if(xInData){
    ...
    const cb = (json)=>{
      ...
      if(json.error){
        ...
        if(timeSpan===1){
          ...
          showerPoints&&waterPoints.forEach((r,i)=>{
  ...
  render() {
    const { data,startTime,endTime, selectedSchool, timeUnit,loading,timeSpan,compare,totalShower,totalWater } = this.state;
    return (
      <div className='chart'>
        <h1>
          设备报修统计
        ...
        <div >
          ...
          <div className='query'>
            <div>
              ...
              <span className='padLR twoLine'>
                <span>热水器总数：{totalShower}</span><br />
          ...
          <div className="lineChartWrapper">
            ...
            >
              ...
              <YAxis axisLine={{stroke:'#ddd'}} domain={['auto', 'auto']} tickLine={false} tick={<CustomizedYAxisTick />} />
              ...
              <Line name={compare?`${DAY[timeSpan]}热水器报修`:'热水器报修'} dataKey="y" stroke="#4aaaef" dot={false} activeDot={{strokeWidth: 2}} />
              <Line name={compare?`${DAY[timeSpan]}饮水机报修`:'饮水机报修'} dataKey="water" stroke="#97da7b" dot={false} activeDot={{strokeWidth: 2}} />
              {compare?<Line name={compare?`${LASTDAY[timeSpan]}热水器报修`:'热水器报修'} dataKey='lastShower' stroke='#ffa312' dot={false} activeDot={{strokeWidth: 2}} />:null}
              {compare?<Line name={compare?`${LASTDAY[timeSpan]}饮水机报修`:'饮水机报修'} dataKey='lastWater' stroke='#ff5555' dot={false} activeDot={{strokeWidth: 2}} />:null}
            </LineChart>
          </div>
        </div>
      </div>
