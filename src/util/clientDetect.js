export function clientDetect() {
  ...
  if (ua.match(/opr\/([\d.]+)/) || window.opera) {
    ...
    engine.ver = browser.ver = opera[1]
    engine.opera = browser.opera = parseFloat(engine.ver)
    if (window.opera) {
      engine.ver = browser.ver = window.opera.version()
      engine.opera = browser.opera = parseFloat(engine.ver)
    }
    result.model = 'opera ' + engine.ver
    ...
    */
    if (/chrome\/(\S+)/.test(ua)) {
      ...
      result.model = 'chrome ' + browser.ver
      ...
      */
      ...
      result.model = 'safari ' + browser.ver
    } else {
      // 近似地确定版本号
      let safariVersion = 1
    ...
    result.model = 'khtml ' + browser.ver
    ...
    */
    ...
    if (/firefox\/(\S+)/.test(ua)) {
      ...
      result.model = 'firefox ' + browser.ver
    }
    ...
    result.model = 'IE ' + browser.ver
  }
  // 检测平台
  let platform = navigator.platform
  if (platform.indexOf('Win') === 0) {
    system.win = true
    result.brand = 'Windows'
