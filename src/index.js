...
const Log = asyncComponent(() => import(/* webpackChunkName: "log" */ "./pages/log/log"))
const Main = asyncComponent(() => import(/* webpackChunkName: "main" */ "./pages/main"))
...
class Layouts extends React.Component {
  ...
  componentDidMount () {
    let logged = getLog(), username = getStore('username'), id = getStore('userId')
    if (logged) {
      let user = {
        name: username,
        id: id
  ...
  hide = (v) => { // hide means if to hide the main content or not
    let loading = this.refs.loading, logged = this.state.logged
  ...
  render() {
    ...
    return (
      <div class='home'>
        <div className='pageLoading hide' ref='loading'>加载中...</div>
        <Switch>
