...
class DepositInfo extends React.Component {
  ...
  fetchDenominations = () => {
    ...
    const cb = json => {
      if (json.error) {
        ...
        if (json.data) {
          let items =
            json.data.rechargeDenominations &&
            json.data.rechargeDenominations[0] &&
            json.data.rechargeDenominations[0].items
          if (!items || items.length < 1) {
            return this.setState({
              schoolError: true,
              schoolErrorMessage:
                '当前学校还未添加充值面额，请先添加充值面额再添加活动～',
              denominations: []
            })
          }
          ...
          if (oldDenos.length > 0) {
            ...
            if (type === 1) {
              oldDenos.forEach(old => {
                ...
                let theDeno = denos.find(
                  each => each.amount === old.denomination
                )
                if (theDeno) {
              ...
              oldDenos.forEach(old => {
                ...
                if (theDeno) {
                  ...
                  old.gifts.forEach((g, ind) => {
                    let newGift = theDeno.gifts.find(
                      gift => gift.id === g.giftId
                    )
                    if (newGift) {
  ...
  checkSchool = v => {
    ...
    if (
      this.state.id &&
      this.state.initialSchool === parseInt(this.state.selectedSchool, 10)
    ) {
      this.fetchDenominations()
    } else {
  ...
  checkCoupons = () => {
    let d = this.state.denominations,
      count = 0
    d.forEach((r, i) => {
      if (r.coupon) {
        count++
  ...
  postInfo = () => {
    ...
    let denos = this.state.denominations,
  ...
  setGift = (gifts, total) => {
    let denos = JSON.parse(JSON.stringify(this.state.denominations)),
      deno = denos[this.state.editingDenomination]
  ...
  render() {
    ...
    const depositPanel =
      ...
      denominations.map((r, i) => (
        <div key={i}>
          <span className="giftCount">¥{r.amount}</span> ————{' '}
          <span className="beforeInput">实际售价(¥):</span>
          <input
            ...
            style={{ width: 100 }}
            value={r.coupon}
            onChange={e => this.changeDenos(e, i)}
            onBlur={this.checkCoupon}
          />
        </div>
      ))
    ...
    const giftPanel =
      ...
      denominations.map((r, i) => (
        <div key={i}>
          <span className="giftCount">¥{r.amount}</span> ————{' '}
          <span className="beforeInput">赠送红包:</span>
          <span className="giftHint">
            {r.giftCount ? `已选${r.giftCount}个红包` : '未选择'}
          </span>
          <a
            className="chooseGift"
            disabled={released ? true : false}
            href=""
            onClick={e => this.chooseGifts(e, i)}
          >
            选择红包
          </a>
        </div>
    ...
    let gifts2modal =
      editingDenomination !== ''
        ? denominations[editingDenomination].gifts
        : gifts
    let total =
      editingDenomination !== ''
        ? denominations[editingDenomination].giftCount
        : 0
      ...
      <div className="infoList depositInfo">
        <ul>
          <li>
            ...
            />
            {schoolError ? (
              <span className="checkInvalid">{schoolErrorMessage}</span>
            ) : null}
          </li>
          <li>
            <p>活动名称:</p>
          ...
          {hasDenomination && selectedSchool && type === '1' && couponError ? (
            <li>
              <p />
              <span className="checkInvalid">请选择优惠面额！</span>
            </li>
          ) : null}
          {hasDenomination && selectedSchool && type === '2' && giftError ? (
            <li>
              <p />
              <span className="checkInvalid">请选择红包！</span>
            </li>
          ) : null}

          {hasDenomination && selectedSchool && type ? (
            <li>
              <p>活动截止日期:</p>
              <input
                disabled={released ? true : false}
                ...
                onBlur={this.checkEndTime}
                required
              />
              {endTimeError ? (
                <span className="checkInvalid">请选择截止日期！</span>
              ) : null}
            </li>
          ) : null}
          ...
          {hasDenomination && selectedSchool && type ? (
            <li>
              ...
              {onlineError ? (
                <span className="checkInvalid">请选择上下线状态！</span>
              ) : null}
            </li>
          ) : null}
        </ul>
        ...
        {hasDenomination && selectedSchool && type ? (
          <div className="btnArea">*充值面额未设置的则不在本次活动范围内</div>
        ) : null}
        {hasDenomination && selectedSchool && type ? (
        ...
        <div>
          <GiftTable
            ...
            total={total}
            gifts={JSON.parse(JSON.stringify(gifts2modal))}
          />
        </div>
      </div>
    )
...
class GiftTable extends React.Component {
  constructor(props) {
    super(props)
    let dataSource = [],
      allTypeData = JSON.parse(JSON.stringify(dataSource)),
      selectedDevice = 'all'
    this.state = {
      allTypeData,
      dataSource,
