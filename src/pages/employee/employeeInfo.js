/* ---------------- state description --------------- */
/* 
   temporary change: (deleted 2018/1/11)
...
class EmployeeInfo extends React.Component {
  ...
  constructor(props) {
    ...
    let id = '',
      ...
      nameErrorMessage = '',
      showSchools = false;
    let mobileErrorMessage = '手机号格式不正确',
      originalMobile = 0,
      schoolError = false;
    this.state = {
      id,
  ...
  buildPrivileges = roles => {
    ...
    let authenStatus = buildAuthenBaseOnfull(
      this.props.full,
      currentPrivileges
    );
    this.setState({
      ...
      if (json.error) {
        ...
        if (json.data) {
          if (this.state.id) {
            ...
            if (id === currentId) {
              // change userName in sessionStorage and store.
              let user = this.props.user;
              user.name = nickName;
              setStore('user', JSON.stringify(user));
              this.props.setUserInfo({ name: nickName });
            }
          }
  ...
  confirm = () => {
    let { account, schoolLimit, roleData, selectedSchools } = this.state;
    if (!account) {
  ...
  closeSchool = () => {
    let { selectedSchools } = this.state,
  ...
  render() {
    ...
    const selectedSchoolItems =
      selectedSchools &&
      selectedSchools.map((r, i) => <span key={i}>{r.name}</span>);
    const selectedRoles = roleData && roleData.filter(r => r.selected === true);
      ...
      <div className="infoList employeeInfo">
        <ul>
          <li>
            ...
            <span>{account}</span>
            {id ? null : (
              <Button type="primary" onClick={this.buildAccount}>
                生成账号
              </Button>
            )}
            {accountError ? (
              <span className="checkInvalid">请先生成账号!</span>
            ) : null}
          ...
          <li>
            ...
            />
            {mobileError ? (
              <span className="checkInvalid">{mobileErrorMessage}</span>
            ) : null}
          </li>
          <li>
            <p>员工身份:</p>
            <a className="mgr10" onClick={this.changeType} href="">
              点击选择
            </a>
            <span className="value mgl10">{selectedRoleItems}</span>
            {roleError ? (
