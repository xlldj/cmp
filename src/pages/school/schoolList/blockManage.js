...
class BlockManage extends React.Component {
  ...
  changePanel = (key) => {
    ...
    let {data} = this.state
    if (typeof i === 'number' && i >= 0 && !data[i].hasOwnProperty('children')) {
      this.setState({
        fetchingIndex: i
      })
  ...
  render (){
    ...
    const panels = data.map((block,index) => {
      ...
      const content = block.children && block.children.map((floor,ind) => {
        const roomDetail = floor.children && floor.children.map((room,i) => {
          let heater = room.devices && room.devices.find((r,index)=>(r.type===1))
          let water = room.devices && room.devices.find((r,index)=>(r.type===2))
          return(
            <div key={`divblock${block.id}floor${floor.residence.id}${i}`} className='roomItem'>
              <span key={`spanblock${block.id}floor${floor.residence.id}${i}`} >{room.residence.name}</span>
      ...
      return (
        <Panel header={head} key={`${index}`} >
          {block.children ? <Tabs tabPosition='left'>{content}</Tabs> : <div className='floorLoading'>加载中...</div>}
        </Panel>
      )
    })
