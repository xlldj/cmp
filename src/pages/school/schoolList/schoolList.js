...
class SchoolList extends React.Component {
  ...
  constructor (props) {
    ...
    this.columns = [{
      ...
      width: '20%',
      render: (text, record) => {
        let percent = record.percent ? (parseFloat(record.percent, 10) * 100).toFixed(1) + '%' : '0%'
        return (
      ...
      width: '8%',
      render: (text, record) => 
        (<Badge text={record.status ? STATUSTEXT[record.status] : '否'} status={record.status ? STATUSICON[record.status] : 'error'} />)
    }, {
    ...
    const cb = (json) => {
        if(json.error){
          ...
          if(json.data){
            ...
            Noti.hintError('该学校不能被禁用','请先将该学校绑定的设备删除再禁用！')
          }
        }
    }
    AjaxHandler.ajax(url, body, cb)    
