...
class AddingBlock extends React.Component {
  ...
  handleBlock = (blockInfo) => {
    ...
    if(blockInfo.residence.buildingType && blockInfo.residence.buildingType.toString()==='1'){
  ...
  checkFloorStart = (e,index)=>{
    ...
    if(!blockInfo[index].start || blockInfo[index].start>100||blockInfo[index].start<0){
      blockInfo[index].roomError = true
      blockInfo[index].roomErrorMsg = '宿舍号越界，下限为0，上限为100，请重新输入!'
      return this.setState({
        blockInfo:blockInfo
      })
    }
  ...
  checkFloorStop =(e,index)=>{
    let blockInfo = JSON.parse(JSON.stringify(this.state.blockInfo))
    if(!blockInfo[index].stop || blockInfo[index].stop>100||blockInfo[index].stop<0||blockInfo[index].stop<blockInfo[index].start){
      blockInfo[index].roomError = true
      blockInfo[index].roomErrorMsg = '宿舍号越界，下限为0，上限为100，请重新输入!'
      return this.setState({
        blockInfo:blockInfo
  ...
  handleRooms = (e,id) => {
    const newBlockInfo = JSON.parse(JSON.stringify(this.state.blockInfo))
    const currFloor = newBlockInfo.find((item,index) => {return item.floorID === id})
    let floorIndex = newBlockInfo.findIndex((item,index) => {return item.floorID === id})
    let roomHeader = currFloor.roomHeader, start = currFloor.start, stop = currFloor.stop
    ...
    newBlockInfo[floorIndex].roomIDs.splice(0,newBlockInfo[floorIndex].roomIDs.length)
  ...
  closeRoom =(e,floorID,r) => {
    ...
    const newBlockInfo = JSON.parse(JSON.stringify(this.state.blockInfo))
    let floorIndex = newBlockInfo.findIndex((item,index) => {return item.floorID === floorID})
    let roomIndex = newBlockInfo[floorIndex].roomIDs.findIndex((item,index) => {return item.fullName === r.fullName})
  ...
  handleSubmit = (e) => {
    ...
    let {id, type,blockName,floorCount, initialName} = this.state , blockInfo=JSON.parse(JSON.stringify(this.state.blockInfo))
    if(type==='0' || !type){
      return this.setState({
        typeError: true
      })
  ...
  changeFloorName = (e,index) => {
    let value = e.target.value,blockInfo=JSON.parse(JSON.stringify(this.state.blockInfo))
  ...
  checkCustomFloorName = (e) => {
    let v = e.target.value.trim(), nextState = {}
    if (!v) {
  ...
  confirmCustomFloorName = () => {
    let {customFloorName} = this.state, nextState = {}
    if (!customFloorName) {
      nextState.customFloorNameError = true
      nextState.customFloorNameErrorMsg = '楼层名不能为空'
      return this.setState(nextState)
  ...
  changeRoomPrefix = (id) => {
    let blockInfo = JSON.parse(JSON.stringify(this.state.blockInfo))
    let floor = blockInfo.find((r) => (r.floorID === id))
    let prefix = floor.prefix, roomHeader = floor.roomHeader
  ...
  render () {
    ...
    if(type&&type==='1'){
      blockItems = blockInfo&&blockInfo.map((item,index) => {
        ...
        const roomItems = roomIDs && roomIDs.map((r,i) => (
          <Tag className='roomNum' closable key={i} onClose={(e) => this.closeRoom(e,item.floorID,r)} >{r.fullName}</Tag>
        ))
        return (
            ...
            <div>
              <div>
                <span className='inputValid'>
                  <input value={item.start} type='number' className='shortInput' step='1' min='1' max='100' placeholder='' id={`floor${index}start`} name={`floor${index}start`} onChange={(e) => this.changeFloorStart(e,item.floorID)} onBlur={(e)=>{this.checkFloorStart(e,index)}} />
                </span>
                <span >
                  开始————结束
                  <input value={item.stop} type='number' className='shortInput addingBlockInput' step='1' min='1' max='100' placeholder='' id={`floor${index}stop`} name={`floor${index}stop`} onChange={(e) => this.changeFloorStop(e,item.floorID)} onBlur={(e)=>{this.checkFloorStop(e,index)}} />
                  {item.fromServer ? null : <Button type='primary' onClick={(e) => {this.togglePrefixBtn(e, item.floorID)}} >添加前缀</Button>}
                  {item.showPrefix ? 
                    <span>
                      <input 
                        value={item.prefix} 
                        onChange={(e) => {this.changePrefix(e, item.floorID)}}
                        placeholder='请输入前缀'
                      />
                      <Button type='primary' onClick={(e) => {this.confirmPrefix(e, item.floorID)}}>确认</Button>
                    </span>
                    : null
                  }
                  <Button type='primary' onClick={(e) => {this.handleRooms(e,item.floorID)}} >批量处理</Button>
                  <Button type='primary' onClick={(e) => {this.toggleCustomRoomBtn(e, item.floorID)}} >单独添加宿舍</Button>
                  {item.showCustomRoom ? 
                    <span>
                      <input 
                        value={item.customRoomName} 
                        onChange={(e) => {this.changeCustomRoomName(e, item.floorID)}}
                        placeholder='请输入宿舍名称'
                      />
                      <Button type='primary' onClick={(e) => {this.confirmCustomRoomName(e, item.floorID)}}>确认</Button>
                    </span>
                    : null
                  }
                  <Button onClick={(e) => {this.deleteFloor(e, item.floorID)}} >删除</Button>
                  {item.roomIDs.length!==0?
                    <span>
                      <Button onClick={(e) => {this.toggleRooms(e,item.floorID)}} >{item.showRooms?'收起':'展开'}</Button>
                    </span>
                    :null
                  }
                </span>
              </div>
              {item.showRooms?<div className='roomsWrapper'>{roomItems}</div>:null}
              {item.roomError?(<div><span className='checkInvalid'>{item.roomErrorMsg}</span></div>):null}
            </div>
          </li>
        )
      })
      ...
      blockItems = blockInfo&&blockInfo.map((item,index) => {
        return (
          <li key={item.floorID} >
            <p>{item.name}名称:</p>
            <input  value={item.name} onChange={(e)=>{this.changeFloorName(e,index)}} onBlur={(e)=>{this.checkFloorName(e,index)}} />
            {item.nameError?(<span className='checkInvalid'>楼层名称不能为空！</span>):null}
          </li>
        )
      })      
    }
    const reset = (<Button onClick={this.reset}>恢复</Button>)
    const floorSwitch = (<Button onClick={this.toggleFloors}>{showFloor?'收起':'展开'}</Button>)
    const customBtn = (
      <li>
        <p>添加自定义楼层:</p>
        <input value={customFloorName} onChange={this.changeCustomFloorName} onBlur={this.checkCustomFloorName} placeholder='请输入楼层名称' />
        <Button type='primary' onClick={this.confirmCustomFloorName} >确认</Button>
        {customFloorNameError ? <span className='checkInvalid'>{customFloorNameErrorMsg}</span> : null}
      </li>
    )
    const addCustom = (
      <li>
        <p></p>
        <Button type='primary' onClick={this.addCustom} >添加自定义楼层</Button>
      </li>
    )
    return (
      <div className='infoList addingBlock'>
        <ul>
          ...
          <li>
            ...
            />
            {typeError?<span className='checkInvalid'>楼栋类型不能为空！</span>:null}
          </li>
          <li>
            <p>楼栋名称:</p>
            <input value={this.state.blockName}
              disabled={id ? true : ''}
              className={id ? 'disabled' : ''} placeholder='楼栋名称' onChange={this.changeBlockName} onBlur={this.checkBlockName} />
            {this.state.blockNameError?(<span className='checkInvalid'>{this.state.blockErrorMessage}</span>):null}
          </li>
          <li>
            <p>楼层数:</p>
            <span className='inputValid'>
              <input value={this.state.floorCount} type='number' step='1' min='1' max='50' placeholder='' id='floorCount' name='floorCount' onChange={this.changeFloorCount} onBlur={this.checkFloorCount} />
            </span>
            <Button type='primary' onClick={this.handleFloor}>批量处理</Button>
            {id?reset:null}
            {this.state.blockInfo.length!==0?floorSwitch:null}
            {this.state.floorCountError?(<span className='checkInvalid'>{floorErrorMsg}</span>):null}
            {this.state.blockError?(<span className='checkInvalid'>楼栋信息不能为空！</span>):null}
          </li>
