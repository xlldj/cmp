...
class RateLimitInfo extends React.Component {
  ...
  postInfo = () => {
    ...
    let time = parseInt(this.state.time, 10), money = parseInt(this.state.money, 10)
  ...
  render () {
    let {id, deviceType, deviceTypeError, businesses,
      schoolId, schoolError, time, timeError, money, moneyError} = this.state
    ...
    return (
      <div className='infoList rateLimitInfo'>
        <ul>
          ...
          <li>
            <p>扣费速率:</p>
              <input type='number' className='shortInput' value={time} onChange={this.changeTime} onBlur={this.checkTime} /> 秒 / 
              <input type='number' className='shortInput' value={money} onChange={this.changeMoney} onBlur={this.checkMoney} /> 元
              {timeError? <span className='checkInvalid' >时间必须为非负整数！</span> : null}
              {moneyError ? <span className='checkInvalid' >金额必须为非负整数！</span> : null}
          </li>       
        </ul>
