...
class DeviceInfo extends React.Component {
  ...
  render() {
    ...
    const rateItems =
      deviceType === 2 &&
      data.water &&
      Object.keys(data.water).map((key, i) => {
        let r = data.water[key]
        return (
          <div className="rateSets" key={i}>
            <span key={`input${i}`}>
              {r.price ? r.price * 100 : ''}分/{r.pulse ? r.pulse : ''}脉冲
            </span>
          </div>
        )
      })
    ...
    let waterMac = data.water
      ? Object.keys(data.water).map((key, i) => (
          <span key={`water${i}`} className="waterMacItem">
            {data.water[key].macAddress}
          </span>
        ))
      : null
      ...
      <div
        className={
          repairs && repairs.length > 0
            ? 'infoBlockList deviceInfo columnLayout'
            : 'infoBlockList deviceInfo'
        }
      >
        <div className={'infoBlock'}>
          <h3>设备信息</h3>
          <ul>
            ...
            <li>
              <p>设备ID:</p>
              {deviceType === 2 ? waterMac : data.macAddress}
            </li>
            <li>
              <p>设备位置:</p>
              {data.location || '暂无'}
        ...
        <div className="btnArea">
          <Button onClick={this.back}>
            {propState && propState.path ? BACKTITLE[propState.path] : '返回'}
          </Button>
        </div>
      </div>
    )
