/**
 * This class is different from 'repairManSelect' that: this does not have callback defined here when confirmed.
 * It should be defined in caller env.
 */
...
class RepairmanTable extends React.Component {
  ...
  confirm = () => {
    ...
    let level = parseInt(this.state.priority, 10)
    if (selectedRowKeys.length < 1) {
      return this.setState({
        hintNoRepairmanSelected: true
      })
    }
    let assignId = selectedRowKeys[0]
    let content = this.state.remark
    this.props.confirm(assignId, level, content)
    /* const body = {
  ...
  render() {
    ...
    const { schoolName, taskCount } = this.props
    const columns = [
      {
        title: '维修员',
        dataIndex: 'name',
