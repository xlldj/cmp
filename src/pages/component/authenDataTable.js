...
export default class AuthenDataTable extends React.Component {
  ...
  setAuthen = (e, rootIndex) => {
    ...
    let node = e.target
    let level = parseInt(node.getAttribute('data-level'), 10),
  ...
  render() {
    ...
    const authenPanel =
      authenStatus &&
      authenStatus.map((r, i) => {
        let level2 =
          r.children &&
          r.children.map((rec, ind) => {
            let level3 =
              rec.children &&
              rec.children.map((record, index) => {
                let level4 =
                  record.children &&
                  record.children.map((item, x) => {
                    return (
                      <div
                        key={`panel${i}level4${item.key}`}
                      ...
                      >
                        <div
                          ...
                          className={
                            item.selected
                              ? 'selected levelHeader level4Header'
                              : 'levelHeader level4Header'
                          }
                        >
                          {item.name}
                        </div>
                        ...
                        >
                          {edit ? (
                            <span className="editBox">
                              <Link to={`/employee/authen/detail/:${item.id}`}>
                                编辑
                              </Link>
                ...
                return (
                  ...
                  >
                    <div
                      ...
                      className={
                        record.selected
                          ? 'selected levelHeader level3Header'
                          : 'levelHeader level3Header'
                      }
                    ...
                    <div
                      key={`panel${i}level3${record.key}children}`}
                      className="levelChilrenWrapper"
                    >
                      {level4}
                    </div>
                  </div>
            ...
            return (
              <div key={`panel${i}level2${rec.key}`} className="levelContent">
                <div
                  ...
                  className={
                    rec.selected
                      ? 'selected levelHeader level2Header'
                      : 'levelHeader level2Header'
                  }
                ...
                <div
                  key={`panel${i}level2${rec.key}children`}
                  className="levelChildrenWrapper"
                >
                  {level3}
                </div>
              </div>
        ...
        return (
          <Panel header={r.name} key={`panel${i}`}>
            <div
              className="levelContent panelWrapper"
              key={`panel${i}tab`}
              onClick={e => {
                this.setAuthen(e, i)
              }}
            >
              <div
                ...
                className={
                  r.selected
                    ? 'levelHeader level1Header selected'
                    : 'levelHeader level1Header'
                }
              ...
              <div
                key={`panel${i}children`}
                className="levelChildrenWrapper level1ChildrenWrapper"
              >
                {level2}
              </div>
            </div>
